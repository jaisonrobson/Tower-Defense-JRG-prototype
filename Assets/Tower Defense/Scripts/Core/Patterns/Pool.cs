...
namespace Core.Patterns
{
	...
	public class Pool<T>
	{
		...
		/// </summary>
		protected readonly Action<T> m_Retrieval;

		/// <summary>
		/// Our reinsertion function
		/// </summary>
		protected readonly Action<T> m_Reinsertion;
		...
		public Pool(Func<T> factory, Action<T> retrieval, Action<T> reinsertion, int initialCapacity)
		{
			if (factory == null)
			{
		...
		public Pool(Func<T> factory, int initialCapacity)
			: this(factory, null, null, initialCapacity)
		{
		}
		...
		{
			return Get(m_Retrieval);
		}

		/// <summary>
		/// Gets an item from the pool, growing it if necessary, and with a specified m_Retrieval function
		/// </summary>
		...
		where T : Component
	{
		...
		/// <param name="initialCapacity">The number of elements to seed the pool with</param>
		public UnityComponentPool(Func<T> factory, Action<T> retrieval, Action<T> reinsertion, int initialCapacity)
			: base(factory, retrieval, reinsertion, initialCapacity)
		{
		}
		...
		{
			if (m_Reinsertion != null)
				m_Reinsertion(element);
	...
	public class GameObjectPool : Pool<GameObject>
	{
		...
		/// <param name="reset">Function to use to reset items when retrieving from the pool</param>
		/// <param name="initialCapacity">The number of elements to seed the pool with</param>
		public GameObjectPool(Func<GameObject> factory, Action<GameObject> retrieval, Action<GameObject> reinsertion, int initialCapacity)
			: base(factory, retrieval, reinsertion, initialCapacity)
		{
		}
	...
	public class AutoGameObjectPrefabPool : GameObjectPool
	{
		...
		public AutoGameObjectPrefabPool(GameObject prefab, Action<GameObject> initialize)
			: this(prefab, initialize, null, null, 0)
		{
		}
		...
		/// <param name="initialize">An initialisation function to call after creating prefabs</param>
		/// <param name="reset">Function to use to reset items when retrieving from the pool</param>
		public AutoGameObjectPrefabPool(GameObject prefab, Action<GameObject> initialize, Action<GameObject> retrieval, Action<GameObject> reinsertion)
			: this(prefab, initialize, retrieval, reinsertion, 0)
		{
		}
		...
		public AutoGameObjectPrefabPool(GameObject prefab, int initialCapacity)
			: this(prefab, null, null, null, initialCapacity)
		{
		}
		...
		public AutoGameObjectPrefabPool(GameObject prefab, Action<GameObject> initialize, Action<GameObject> retrieval, Action<GameObject> reinsertion,
		                                int initialCapacity)
			: base(DummyFactory, retrieval, reinsertion, 0)
		{
		...
		where T : Component
	{
		...
		/// <param name="initialize">An initialisation function to call after creating prefabs</param>
		/// <param name="reset">Function to use to reset items when retrieving from the pool</param>
		public AutoComponentPrefabPool(T prefab, Action<T> initialize, Action<T> retrieval, Action<T> reinsertion)
			: this(prefab, initialize, retrieval, reinsertion, 0)
		{
		}
		...
		/// <param name="reset">Function to use to reset items when retrieving from the pool</param>
		/// <param name="initialCapacity">The number of elements to seed the pool with</param>
		public AutoComponentPrefabPool(T prefab, Action<T> initialize, Action<T> retrieval, Action<T> reinsertion, int initialCapacity)
			: base(DummyFactory, retrieval, reinsertion, 0)
		{
			// Pass 0 to initial capacity because we need to set ourselves up first
			// We then call Grow again ourselves
